...
def capture_stereoimu(data_path, log):
    ...
    move_arm_stereoimu()
    pull_data("mv", os.path.join(
        shared_dir, stereoimu_src_dir), stereoimu_path)
...
def move_arm_fergb(log=[]):
 # 获取标定数据-P3-静态内外参标定
    ...
    while (x < 16):
        ...
        cali.FE_capture(self.save_path, self.no[x], self.config["DevicePath"]
                        ["FE_path"], self.config["Command"]["FE_capture"])
        cali.RGB_capture(self.save_path, self.no[x], self.config["DevicePath"]
                         ["RGB_path"], self.config["Command"]["RGB_capture"])
        '''
        os.chdir(path_TOF)
